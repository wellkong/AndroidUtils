...
//downloadUtil.start();
    //7.0系统以上 会报出错误Android 7.0 FileUriExposedException 的处理
    //1.在 res/xml 目录下创建文件 provider_paths.xml 内容如下：
    //<?xml version="1.0" encoding="utf-8"?>
//<paths xmlns:android="http://schemas.android.com/apk/res/android">
//    <root-path
//        name="root_path"
//        path="." />
//    <external-files-path name="apk" path="app/" />
//</paths>
    //2.在AndroidManifest.xml清单文件中的Application中加入如下代码：
...
//        </provider>
    //3.步骤2设置了authorities为应用包名，所以必须把包名设置进来
    //调用方法setPackageName
...
public class DownloadUtil {
                    ...
                    if (filePath != null) {
                        if (filePath.contains(context.getPackageName())) {
                            if (filePath.endsWith("apk")) {
                                AppUtils.installAPK(context, filePath.trim().substring(7),getPackageName());
                            } else {
    ...
    public static void setPackageName(String packageName) {
        DownloadUtil.packageName = packageName;
    }
}
